...
class PNFW_Notifications_iOS extends PNFW_Notifications {
	...
	protected function raw_send($tokens, $title, $user_info) {
		...
		if (empty($certificate)) {
			pnfw_log(PNFW_IOS_LOG, __('iOS SSL certificate is not correctly set.', 'push-notifications-for-wp'));
	    	return 0;
		}
		
		if (empty($passphrase)) {
			pnfw_log(PNFW_IOS_LOG, __('iOS SSL certificate password is not correctly set.', 'push-notifications-for-wp'));
	    	return 0;
		}
		
		if (!file_exists($certificate)) {
			pnfw_log(PNFW_IOS_LOG, __('iOS SSL Certificate does not exists.', 'push-notifications-for-wp'));
			return 0;
		}
		...
		try {
			...
			$push->setLogger(new PNFW_ApnsPHP_Logger(PNFW_IOS_LOG));
