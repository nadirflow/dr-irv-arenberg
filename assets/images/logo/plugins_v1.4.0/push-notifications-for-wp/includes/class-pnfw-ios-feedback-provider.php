...
class PNFW_iOS_Feedback_Provider {
	...
	function run() {
		if (!$this->is_active()) {
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("Scheduled Feedback Provider.", 'push-notifications-for-wp'));
	...
	function stop() {
		if ($this->is_active()) {
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("Un-scheduled Feedback Provider.", 'push-notifications-for-wp'));
	...
	function feedback_provider() {
		pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("Feedback Provider is running.", 'push-notifications-for-wp'));
		...
		if (empty($certificate)) {
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("iOS SSL certificate is not correctly set.", 'push-notifications-for-wp'));
	    	return;
		}
		
		if (empty($passphrase)) {
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("iOS SSL certificate password is not correctly set.", 'push-notifications-for-wp'));
	    	return;
		}
	
		if (!file_exists($certificate)) {
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, __("iOS SSL Certificate does not exists.", 'push-notifications-for-wp'));
			return;
		}
		...
		try {
			...
			$feedback->setLogger(new PNFW_ApnsPHP_Logger(PNFW_FEEDBACK_PROVIDER_LOG));
		...
		foreach ($devices as &$device) {
			...
			pnfw_log(PNFW_FEEDBACK_PROVIDER_LOG, sprintf(__("The Feedback Provider removed iOS token of user %s: %s.", 'push-notifications-for-wp'), $user_id, $device['deviceToken']));
